	...
	let routes = {
		'get': {
			...
			'/redzone': (req, res, next) => {
				...
				console.log("INFO : Checking DB for game and player names")
				if(h.isNameValid(req.query.gamename, 'game')) {
			...
			'/getPlayers': (req, res, next) => {
				...
				h.getPlayers().then(players => {
					...
					console.log("INFO : Helper function getPlayers() returned to router with : " + JSON.stringify(players));
